...
- [프로젝트 개요](#프로젝트-개요)
- [주요 기능](#주요-기능)
- [ETL 파이프라인 및 시스템 아키텍처](#etl-파이프라인-및-시스템-아키텍처)
...
**Deep Nexus**는 사내에 흩어져 있는 다양한 정형 데이터(RDB)와 비정형 데이터(보고서, 기획서, 사내 규정 등)를 하나의 인터페이스에서 자연어로 질의하고 통합된 답변을 받을 수 있는 **RAG(Retrieval-Augmented Generation) 기반 AI 챗봇**입니다.
...
```
[데이터 소스]                    [ETL 파이프라인]                  [서빙 레이어]
                                                              
 ┌──────────────┐              ┌──────────────────┐           ┌──────────────────┐
 │  RDB (ERP)   │──────────▶  │  스키마 분석      │──────────▶│  PostgreSQL      │
 │  인사/급여 등 │              │  DDL 벡터화       │           │  + pgvector      │
 └──────────────┘              └──────────────────┘           │  (HNSW Index)    │
                                                              └────────┬─────────┘
 ┌──────────────┐              ┌──────────────────┐                    │
 │  비정형 문서  │──────────▶  │  문서 파싱        │──────────▶        │
 │  보고서/규정  │              │  청킹 & 임베딩    │           ┌───────▼─────────┐
 │  Google Drive │              │  (KURE-v1 1024d) │           │  FastAPI Server │
 └──────────────┘              └──────────────────┘           │  + LangGraph    │
                                                              └───────┬─────────┘
                                                                      │
                                                              ┌───────▼─────────┐
